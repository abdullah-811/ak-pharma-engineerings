        ...
        <div className="container-page grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {products.map((p) => (
            <Link
              ...
              to="/products/$slug"
              params={{ slug: p.slug }}
              className="group bg-white rounded-xl overflow-hidden border border-border hover:shadow-brand hover:border-brand/30 transition flex flex-col"
            >
              ...
              <div className="p-6 flex flex-col flex-1">
                <h3 className="font-display font-bold text-brand-deep text-lg">{p.name}</h3>
                <p className="mt-2 text-sm text-muted-foreground line-clamp-3 flex-1">{p.short}</p>
